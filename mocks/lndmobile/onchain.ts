...
export const newAddress = jest.fn(
  async (
    type: lnrpc.AddressType = lnrpc.AddressType.WITNESS_PUBKEY_HASH,
  ): Promise<lnrpc.NewAddressResponse> => {
    const response = lnrpc.NewAddressResponse.create({
      address: "tb1qsl4hhqs8skzwknqhwjcyyyjepnwmq8tlcd32m3",
    });
    return response;
  },
);
...
export const sendCoins = jest.fn(
  async (address: string, sat: number): Promise<lnrpc.SendCoinsResponse> => {
