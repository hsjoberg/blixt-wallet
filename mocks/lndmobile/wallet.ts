...
export const initWallet = jest.fn(
  async (seed: string[], password: string): Promise<lnrpc.InitWalletResponse> => {
    const response = lnrpc.InitWalletResponse.create({});
    return response;
  },
);

export const unlockWallet = jest.fn(
  async (password: string): Promise<lnrpc.UnlockWalletResponse> => {
