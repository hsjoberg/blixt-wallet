...
export default function ReceiveSetup({ navigation }: IReceiveSetupProps) {
  ...
  const createBlixtLspInvoice = async () => {
    ...
    console.log(result);
    try {
      const connectPeerResult = await connectPeer(result.peer);
    } catch (e) {
      console.log("Error connectPeer", result.peer);
      console.log(e);
    }
    ...
    const message =
`In order to accept a payment, a channel has to be opened.

This will require a one-time fee of approximately ${approxFeeFormatted} (${approxFeeFiat}).`;
    Alert.alert(
      "Channel opening",
      message,
      [{
  ...
  if (shouldUseDunder) {
    ...
    noticeText = (
      <>
        A payment channel by Dunder LSP will be opened when you receive payment for this invoice.
        {" \n"}
        <TextClickable style={{
          fontSize: 14,
          lineHeight: 22,
